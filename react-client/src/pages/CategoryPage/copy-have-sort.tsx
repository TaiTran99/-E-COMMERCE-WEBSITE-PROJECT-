...
const CategoryPage = () => {
  ...
  const sortType = search.get("sortType")|| ''  ;
...
const getProducts = async ()=>{
  let url = globalConfigs.urlAPI + `/v1/products/client/getall?cat_id=${id}&page=${int_page}&limit=${int_limit}`
  if (sortBy != '' && sortType != '' ){
...
const {data: products, isSuccess, isError, isLoading, error} = useQuery({ 
  ...
  queryKey: ['categories',id,int_page,int_limit,sortBy,sortType], 
  queryFn: getProducts 
});
...
type TPagination = {
  ...
  if(isSuccess){
    ...
    for (let index = 1; index <= products.data.totalPages; index++) {
      let url =`/categories/${id}?page=${index}`
      if (sortBy != '' && sortType != '' ){
        url+=`&sortBy=${sortBy}&sortType=${sortType}`
      }
  ...
  return (
    <div>
      ...
      <div className="filters my-5 flex gap-x-5">
          <Link to={`/categories/${id}?sortBy=price&sortType=ASC`}>Giá tăng dần</Link>
          <Link to={`/categories/${id}?sortBy=price&sortType=DESC`}>Giá giảm dần</Link>
      </div>
